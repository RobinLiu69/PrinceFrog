...
## IF turned on when calling attack method the projectile will spawned. Otherwise, it will spawn in certain conditions.(See in Trigger)
...
## If there are mutiple targets and it is turned on, will trigger an error.
...
## Radius is based on the owner size. etc 150% (1.5), 50% (0.5) of thw owner size.
...
func _ready() -> void:
	...
	assert("existing_time" in test_projectile, " Projectile don't have the required variable < existing_times >")
	assert(test_projectile.has_method("stun") and stun_active, " Projectile don't have the stun method under it")
	assert(test_projectile.has_method("slow") and slow_active, " Projectile don't have the slow method under it")
	...
	assert(attack_type != "all" and facing_the_target_rotation == false, " Can't facing mutiple targets.")
...
func find_the_nearest_target(target_list: Array[CharacterBody2D]) -> CharacterBody2D:
...
func find_the_farthest_target(target_list: Array[CharacterBody2D]) -> CharacterBody2D:
...
func find_random_target(target_list: Array[CharacterBody2D]) -> CharacterBody2D:
	return target_list.pick_random()

func get_target_list() -> Array[CharacterBody2D]:
...
func spawn_projectile(from: CharacterBody2D, set_position: Vector2 = Vector2()) -> CharacterBody2D:
	...
	var target_list: Array[CharacterBody2D] = get_target_list()
	var targets: Array[CharacterBody2D] = [null]
	match attack_type:
		"nearest":
	...
	if spawn_at_root_scene:
		...
		add_child(instance)
		instance.owner = self
	if defult_position == "onwer" and set_position == Vector2.ZERO:
